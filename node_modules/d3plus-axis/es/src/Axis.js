  ...
  */
  Axis.prototype.render = function render (callback) {
    ...
    if (this._scale === "ordinal" && this._domain.length > range.length) {
      range = d3Range(this._domain.length).map(function (d) { return this$1._size * (d / (this$1._domain.length - 1)) + range[0]; });
    }
    ...
    if (this._scale !== "band" && textData.length) {
      ...
      var firstB = min([this._getPosition(first.d) - first[width] / 2, range[0] - wBuff]);
      if (firstB < range[0]) {
      ...
      var lastB = max([this._getPosition(last.d) + last[width] / 2, range[lastI] + wBuff]);
      ...
      if (range.length > 2) { range = d3Range(this._domain.length).map(function (d) { return this$1._size * (d / (range.length - 1)) + range[0]; }); }
